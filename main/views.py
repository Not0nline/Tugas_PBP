...
def show_main(request):
    ...
    datas = Product.objects.filter(user=request.user)
...
def delete_product_ajax(request, product_id):
    if request.method == 'POST':
        try:
            ...
            if product.user == request.user:  # Check if the product belongs to the current user
                product.delete()
                return HttpResponse(b"DELETED", status=200)
            else:
                return HttpResponse(b"UNAUTHORIZED", status=403)  # User is not authorized to delete this product
